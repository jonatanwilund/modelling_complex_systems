...
def parseArguments():
    parser = argparse.ArgumentParser(
        description="Simulate the Lecture 10 city Markov chain"
    )
    parser.add_argument(
        "--steps",
        type=int,
...
def formatVector(vector, decimals):
    entries = [
        f"{label} = {value:.{decimals}f}" for label, value in zip(nodeLabels, vector)
    ]
    return ", ".join(entries)
...
def printTrajectory(trajectory, decimals):
    ...
    for step, state in enumerate(trajectory):
        ...
        print(
            f"  t = {step:2d}: {formatVector(state, decimals)} | total = {totalMass:.{decimals}f}"
        )
