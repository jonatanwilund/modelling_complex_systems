...
def parseArguments():
    parser = argparse.ArgumentParser(
        description="Compute damped PageRank on the Lecture 11 graph"
    )
    parser.add_argument(
        "--alpha",
        nargs="+",
    ...
    parser.add_argument(
        "--tol", type=float, default=defaultTolerance, help="L1 convergence tolerance"
    )
    parser.add_argument(
        "--decimals", type=int, default=6, help="number of decimals in printed scores"
    )
...
def main():
    ...
    for alpha in args.alpha:
        ranks, nIterations, error = dampedPageRank(
            graph, alpha, args.iterations, args.tol
        )
