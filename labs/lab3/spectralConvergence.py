...
def stationary_distribution(P): 
    ...
    pi = np.real(eigvecs[:, idx]) # Using np.real() since there can be small unwanted imaginary parts   

    # linalg.eig can arbitrarily return v or -v. We want the one with positive elements, so: 
    if np.sum(pi) < 0: 
        pi = -pi

    pi = pi / sum(pi) # Normalize with L1 norm (here it actually is L1 bc of PF-theorem) to ensure elements sum to 1 
...
def main(): 
    ...
    for label, P in matrices.items(): 
        ...
        t = np.arange(T + 1)
        plt.semilogy(t, distances, label=f'{label}: Tot. var. distance', )

        comparison = distances[0] * (mu ** t)
        plt.semilogy(t, comparison, "--", label=f"{label}: " + r"proportional to $\mu^t$")
