...
def fun_large_grid(n, N, seed=12):
    ...
    for cy, cx in centers:
        ...
        if N >= 2:
            grid[(r > ring_radius - tail_width) & (r <= ring_radius)] = _clip_state(N // 3, N)
            grid[(r > ring_radius - 2 * tail_width) & (r <= ring_radius - tail_width)] = _clip_state(2 * N // 3, N)
            grid[(r > ring_radius - 3 * tail_width) & (r <= ring_radius - 2 * tail_width)] = _clip_state(N, N)
