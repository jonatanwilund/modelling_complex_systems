...
def plot_snapshot(position_history, theta_history, eta, t):
    ...
    plt.xlabel("x")
    plt.ylabel("y")
    plt.title(f"Vicsek model snapshot for N = {N} particles at t = {t}, eta = 2$\pi$, \n R = {R}, $v_0$ = {v0}")
...
def plot_phi(Phi_values, eta):
    ...
    plt.title(
        f"Order parameter vs time, eta = 2$\pi$"
    )
...
def plot_average_phi_vs_eta(N, R, v0, T):
    ...
    for k, eta in enumerate(eta_values):
        ...
        _, _, Phi_values = run_simulation(
            N, R, eta, v0, T
        )

        average_phi_values[k] = time_average_phi(
            Phi_values
        )
    ...
    plt.title(
        "Vicsek model: average alignment vs noise"
    )
